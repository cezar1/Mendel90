...
module rpi_camera_front_stl() {
    ...
    difference() {
        ...
        translate([0, 3+pi_cam_centreline + sensor_length / 2, shelf - pi_cam_thickness])             // clearance for sensor connector
            cube([9+pi_cam_turret + 2 * clearance, sensor_length-3, 2 * pi_cam_front_clearance], center = true);
...
blinkt_depth=7;
blink_extend=20;
module blinkt_bracket()
{
    difference()
                {
                color([1,0,0])cube([blinkt_width+2*wall, blinkt_depth, blinkt_length+2*wall],center=true);
                
                cube([blinkt_width, blinkt_depth+0.1, blinkt_length],center=true);
                }
}
...
module rpi_light_clamp_stl() {
    ...
    thickness = blinkt_depth +2 *wall;
    width=blinkt_width+2*wall;
    overlap = 1;
    length = blinkt_width + 2 * wall;
    gap = blinkt_width + clearance;
    ...
    difference() {
        union() {
            connector_height=5;
            translate([2,-bar_y_offset,connector_height/2])cube([blink_extend,blinkt_depth/2,connector_height],center=true);
            translate([-length / 2+12-blink_extend, - bar_y_offset, blinkt_length/2+wall])
            {
